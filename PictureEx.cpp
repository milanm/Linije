...
CPictureEx::CPictureEx()
{
	...
	m_bIsPlaying       = FALSE;
	m_bIsGIF		   = FALSE;
	m_clrBackground = RGB(49,65,132); // white by default
	m_nGlobalCTSize    = 0;
	m_nCurrOffset	   = 0;
	m_nCurrFrame	   = 0;
	m_nDataSize		   = 0;
	...
	misPreko = false; kliknuto = false;
	int trenutno_ucitani_oblik = -1;
...
void CPictureEx::ThreadAnimation()
{
	...
	if (m_arrFrames[m_nCurrFrame].m_nDisposal == 2)
	{
		...
		// if there is a background DC present, bitblt its content
		if (m_hBackDC)
		{
			BitBlt(m_hMemDC, m_arrFrames[m_nCurrFrame].m_frameOffset.cx,
				m_arrFrames[m_nCurrFrame].m_frameOffset.cy,
	...
	while (!m_bExitThread)
	{
		...
		// init the screen for the first frame,
			...
			if (m_hBackDC)
			{
				BitBlt(m_hMemDC, 0,	0, m_PictureSize.cx, m_PictureSize.cy,
...
void CPictureEx::OnPaint()
{
	...
	if (nPaintWidth > 0)
	{
		...
		//::TransparentBlt( dc.m_hDC, 0, 0, nPaintWidth, nPaintHeight, 
		//m_hMemDC, m_PaintRect.left, m_PaintRect.top, m_PaintRect.left, m_PaintRect.top, m_clrBackground);
	...
	else
	{
		...
		//::TransparentBlt( dc.m_hDC, 0, 0, m_PictureSize.cx, m_PictureSize.cy, 
		//m_hMemDC, 0,0, m_PictureSize.cx, m_PictureSize.cy, m_clrBackground);
	};
}
...
BOOL CPictureEx::PrepareDC(int nWidth, int nHeight)
{
	...
	// fill the background
	if(!IsWindowEnabled()) m_clrBackground = RGB(0,0,0);
	else m_clrBackground = RGB(49,65,132);
	HBRUSH hBrush = CreateSolidBrush(m_clrBackground);
...
BOOL CPictureEx::SetBackgroundDC(HDC hSourceDC, int nXOffset, int nYOffset, int nWidth, int nHeight)
{
	HDC hWinDC = ::GetDC(m_hWnd);
	if (!hWinDC) return FALSE;
	
	m_hBackDC = CreateCompatibleDC(hWinDC);
	if (!m_hBackDC) 
	{
		::ReleaseDC(m_hWnd, hWinDC);
		return FALSE;
	...
	m_hBackBitmap = CreateCompatibleBitmap(hWinDC, nWidth, nHeight);
	if (!m_hBackBitmap) 
	{
		::ReleaseDC(m_hWnd,hWinDC);
		::DeleteDC(m_hBackDC);
	...
	m_hBackOldBitmap = reinterpret_cast<HBITMAP> (SelectObject(m_hBackDC, m_hBackBitmap));
	
	BitBlt(m_hBackDC, 0, 0, nWidth, nHeight, hSourceDC, nXOffset, nYOffset, SRCCOPY);
	BitBlt(m_hMemDC, 0, 0, nWidth, nHeight, m_hBackDC, 0, 0, SRCCOPY);

	return TRUE;
}

BOOL CPictureEx::SetBackgroundDC(LPCTSTR lpBitmapName, int nXOffset, int nYOffset, int nWidth, int nHeight)
{
	HDC hWinDC = ::GetDC(m_hWnd);
	if (!hWinDC) return FALSE;
	...
	HBITMAP hBitmap = LoadBitmap(AfxGetInstanceHandle(), lpBitmapName);
	if (!hBitmap) 
	{
		::DeleteDC(hMemDC);
		return FALSE;
	...
	BOOL bRet = SetBackgroundDC(hMemDC, nXOffset, nYOffset, nWidth, nHeight);
...
int CPictureEx::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	if(trenutno_ucitani_oblik!=oblik)
	{ 
	  if (Load(MAKEINTRESOURCE(oblik),_T("GIF"))) Draw();
	  trenutno_ucitani_oblik = oblik;
	}
...
void CPictureEx::OnLButtonDown(UINT nFlags, CPoint point)
{
	if(!kliknuto)
	{ 
		if(trenutno_ucitani_oblik != (oblik+2))
		{
			if (Load(MAKEINTRESOURCE(oblik+2),_T("GIF"))) Draw(); 
			trenutno_ucitani_oblik = oblik+2;
		}
		kliknuto = true;
	}
	else
	...
	CWnd *parent = GetParent(); parent->PostMessage(WM_PORUKA,(WPARAM) cvor.uzmi_x(), (LPARAM) cvor.uzmi_y());
	
	CStatic::OnLButtonDown(nFlags, point);
}

void CPictureEx::OnKillFocus(CWnd* pNewWnd)
{
...
void CPictureEx::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
	CStatic::OnActivate(nState, pWndOther, bMinimized);

	if(nState == WA_INACTIVE)
	{
		if(!kliknuto){ if (Load(MAKEINTRESOURCE(oblik),_T("GIF"))) Draw(); }
		else { if (Load(MAKEINTRESOURCE(oblik+2),_T("GIF"))) Draw(); }
	}
}

void CPictureEx::upisi_oblik(int cs)
{
	oblik = cs; trenutno_ucitani_oblik = cs;
}

int CPictureEx::uzmi_oblik(void)
{
	return oblik;
}

UINT CPictureEx::OnNcHitTest(CPoint point)
{
	return HTCLIENT;
}
void CPictureEx::OnMouseMove(UINT nFlags, CPoint point)
{
	if (misPreko) // Cursor currently over control
	{
		CRect rect;
		GetClientRect(rect);
	...
	else // Cursor has left control area
	{
		if(!kliknuto)
		{
			if(trenutno_ucitani_oblik!=(oblik+1))
			{ 
				if (Load(MAKEINTRESOURCE(oblik+1),_T("GIF"))) Draw(); 
				trenutno_ucitani_oblik = oblik+1;
			}
		}
		misPreko = TRUE; SetCapture();
	}
}
